...
function createTodo(todos) {
    ...
    return todos.map((el, index) => {
        ...
        <li class="item" id=${index} >
            <input class="editInput" id=${index} value="${el.text}" disabled />
          <div class="wrapIcon">
...
function editTodo(evt) {
    ...
    if (evt.target.closest(".editBtn")?.className === "editBtn") {

        const itemID = Number(evt.target.closest(".item").id);
        const item = evt.target.closest(".item")
        const editInput = item.querySelector(".editInput")
        // const todoText = evt.target.parentElement.querySelector(".editInput")
        // console.log('itemID:', itemID)
        console.log('editInputValue:', editInput)

        editInput.removeAttribute("disabled");
        editInput.focus();
    }

    // console.log("evt.target.className: ", evt.target.closest(".editBtn")?.className)



    // const editId = evt.target.closest(".item").id;
    // console.log('evt.targetItemID:', evt.target.closest(".item").id)
